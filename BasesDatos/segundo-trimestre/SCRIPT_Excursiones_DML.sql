...
CREATE DATABASE IF NOT EXISTS Excursiones_DOS; 
USE Excursiones_DOS;
...
-- 1
INSERT INTO RUTA VALUES('R09', 'VISITA INVERNADEROS', 'INVERNADERO POR DENTRO Y FUNCIONAMIENTO', 50, 'FACIL');

INSERT INTO excursion(CODIGO_E, NOMBRE, FECHA, H_INICIO, H_FIN, N_PLAZAS, EMPLEADO) VALUES('EX013', 'VISITA A LAS CANTERAS', '2022-02-27', '08:00:00', '15:00:00', 20, '88888888O');
INSERT INTO excursion(CODIGO_E, NOMBRE, FECHA, N_PLAZAS, RUTA, EMPLEADO) VALUES('EX014', 'INVERNADERO CON ENCANTO', '2022-06-06', 35, 'R09', '33333333M');
INSERT INTO excursion(CODIGO_E, NOMBRE, FECHA, H_INICIO, H_FIN) VALUES('EX015', 'PLAYA DE LOS MUERTOS DESDE OTRA VISTA', '2022-08-05', '11:00:00', '16:00:00');

-- 2
UPDATE EXCURSION SET N_PLAZAS = N_PLAZAS + 5 WHERE month(FECHA) BETWEEN 3 AND 5 AND year(FECHA) = 2020;

-- 3
DELETE FROM RECURSO WHERE CODIGO NOT IN(SELECT RECURSO FROM USA);

-- 4
UPDATE EMPLEADO SET SALARIO = SALARIO + 150 WHERE DNI IN(SELECT EMPLEADO FROM EXCURSION WHERE year(FECHA) = 2020 GROUP BY EMPLEADO HAVING COUNT(*) = 3);

-- 5
INSERT INTO HISTORICO_EXCURSIONES SELECT *, timediff(H_FIN, H_INICIO) FROM EXCURSION;
SELECT * FROM HISTORICO_EXCURSIONES;

-- 6
UPDATE RECURSO SET STOCK = STOCK - 5 WHERE CODIGO IN(SELECT RECURSO FROM USA GROUP BY RECURSO HAVING COUNT(*) >= 2);

-- 7
INSERT INTO RANKING_EXCURSIONES SELECT CODIGO_E, NOMBRE, COUNT(USUARIO) FROM EXCURSION LEFT JOIN RESERVA R ON EXCURSION = CODIGO_E GROUP BY CODIGO_E;

-- 8
LOCK TABLE USUARIO READ;

-- 9
START TRANSACTION;
DELETE FROM RESERVA WHERE FORMA_PAGO = 'BIZUM';
ROLLBACK;