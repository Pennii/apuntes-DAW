CREATE DATABASE IF NOT exists campeonatoAtletismo;
use campeonatoAtletismo;
...
references UNIVERSIDAD(codigo) on delete set null on update cascade);
...
references ATLETA (dorsal) on delete cascade on update cascade,
constraint com_cod_FK foreign key(codigo_pru)
references PRUEBA (codigo) on delete cascade on update cascade);
...
-- -----------------------------------
-- 1
SELECT codigo, nombre, entrenador FROM UNIVERSIDAD ORDER BY 2;

-- 2
SELECT CODIGO, DISTANCIA, OBSTACULOS, CATEGORIA, date_format(FEC_HORA, "%d %M %Y") FROM PRUEBA ORDER BY 5;

-- 3 
SELECT UNIVERSIDAD, COUNT(*) FROM ATLETA GROUP BY UNIVERSIDAD ORDER BY 2 DESC;

-- 4
SELECT A.NOMBRE, APELLIDOS, U.NOMBRE FROM ATLETA A INNER JOIN UNIVERSIDAD U ON CODIGO = UNIVERSIDAD WHERE COMUNIDAD = "ANDALUCIA";

-- 5
SELECT CODIGO_PRU, NOMBRE, APELLIDOS, POSICION FROM ATLETA INNER JOIN COMPETIR ON DORSAL = DORSAL_ATL WHERE POSICION <= 3 ORDER BY 1, 4;
 
 -- 6
 SELECT DISTANCIA, COUNT(*) FROM PRUEBA INNER JOIN COMPETIR ON CODIGO = CODIGO_PRU GROUP BY DISTANCIA ORDER BY 1 DESC;
 
 -- 7
 SELECT CODIGO_PRU, DORSAL_ATL, MARCA_PERSONAL, REC_UNIVERSITARIO, timediff(REC_UNIVERSITARIO, MARCA_PERSONAL) FROM COMPETIR INNER JOIN PRUEBA ON CODIGO = CODIGO_PRU WHERE POSICION = 1;
 
 -- 8
 SELECT UNIVERSIDAD, SUM(PUNTOS) FROM COMPETIR INNER JOIN ATLETA ON DORSAL = DORSAL_ATL  GROUP BY UNIVERSIDAD order by 2;
 
 -- 9
 SELECT A.NOMBRE, APELLIDOS FROM ATLETA A INNER JOIN COMPETIR ON DORSAL = DORSAL_ATL INNER JOIN PRUEBA ON CODIGO = CODIGO_PRU WHERE DISTANCIA = 3000 AND CATEGORIA IN("M","F") AND MARCA_PERSONAL IS NULL ORDER BY APELLIDOS;
 
 -- 10
 SELECT A.NOMBRE, APELLIDOS, CONCAT(truncate(datediff(CURDATE(), FEC_NAC)/365, 0)," AÑOS") as años  FROM ATLETA A INNER JOIN UNIVERSIDAD ON UNIVERSIDAD = CODIGO WHERE PROVINCIA LIKE "A%";
 
 -- 11
 SELECT U.NOMBRE, COUNT(*) FROM UNIVERSIDAD U INNER JOIN PODIUM ON UNIVERSIDAD = CODIGO GROUP BY UNIVERSIDAD ORDER BY 2 DESC;
 
 -- 12
 SELECT CONCAT(DORSAL, " ", NOMBRE, " ", APELLIDOS), COUNT(*) FROM ATLETA INNER JOIN COMPETIR ON DORSAL = DORSAL_ATL where posicion <=3 GROUP BY DORSAL HAVING COUNT(*) >= 2 ORDER BY 2;
 ...
 -- ----------------
 -- 1
 INSERT INTO ATLETA VALUES(0200,"JUAN", "PEREZ SORIANO","M","1998/02/01","UIB");
 
 -- 2
 UPDATE ATLETA SET FEC_NAC = "1998-03-01" WHERE DORSAL = "0157";
 
 -- 3
 DELETE FROM COMPETIR WHERE DORSAL_ATL = "0151" AND CODIGO_PRU = "400VM";
 
 -- APARTADO B
 -- ------------
 -- 1
 INSERT INTO COMPETIR VALUES("0155", "1500F", NULL, NULL, NULL);
 
 -- 2
 UPDATE ATLETA SET APELLIDOS = "PEREZ MARTINEZ", FEC_NAC = "1995-2-1", UNIVERSIDAD = "UGR" WHERE DORSAL = "0167";
 
 -- 3
 DELETE FROM COMPETIR WHERE POSICION IS NULL;
 
 -- 4
 UPDATE PRUEBA SET FEC_HORA = adddate(FEC_HORA, 15);
 
 -- 5
 UPDATE COMPETIR SET PUNTOS = PUNTOS + 5 WHERE POSICION >= 2;
 ...
 CREATE TABLE PODIUM(
    ...
    PSOICION TINYINT,
    MARCA_PERSONAL TIME,
    CONSTRAINT PD_PK PRIMARY KEY (NOMBRE, APELLIDO, PRUEBA));
INSERT INTO PODIUM SELECT NOMBRE, APELLIDOS, CODIGO_PRU, UNIVERSIDAD, POSICION, MARCA_PERSONAL FROM ATLETA INNER JOIN COMPETIR ON DORSAL = DORSAL_aTL WHERE POSICION <= 3; 

 -- 7
DELETE FROM COMPETIR WHERE DORSAL_ATL IN(SELECT DORSAL_ATL FROM (SELECT * FROM COMPETIR) AS COMP INNER JOIN PRUEBA ON CODIGO = CODIGO_PRU WHERE TIMEDIFF(MARCA_PERSONAL, REC_UNIVERSITARIO) >= "00:01:30");
SELECT DORSAL_ATL FROM (SELECT * FROM COMPETIR) AS COMP INNER JOIN PRUEBA ON CODIGO = CODIGO_PRU WHERE TIMEDIFF(MARCA_PERSONAL, REC_UNIVERSITARIO) >= "00:01:30";