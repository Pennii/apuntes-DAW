...
-- 7
CREATE TABLE coste_hoteles(
	codHotel int primary key,
    direccion VARCHAR (60),
    totDias int,
    totGanado FLOAT);
Start TRANSACTION;
INSERT INTO coste_hoteles (SELECT a.codHotel, direccion, sum(numDias), sum(numDias * pagoxhora * 8) FROM asignacion a INNER JOIN empleado e ON e.codempleado = a.codEmpleado INNER JOIN hotel h ON h.codHotel = a.codHotel GROUP BY a.codHotel);
SELECT * FROM coste_hoteles;
