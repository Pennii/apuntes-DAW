...
-- 3
START TRANSACTION;
DELETE FROM PEDIDO WHERE NUM_PED  IN(SELECT NUM_PED FROM (SELECT * FROM PEDIDO) AS PED WHERE MONTH(FECHA_PED) != 9 OR year(FECHA_PED) != 2018);
ROLLBACK;
...
-- 6
UPDATE DETALLE_PEDIDO SET TOTAL_LINEA = (SELECT CANTIDAD * PRECIO FROM PLATO INNER JOIN DETALLE_PEDIDO ON COD_PLATO = PLATO);
SELECT NUM_PEDIDO, NUM_LINEA, CANTIDAD * PRECIO FROM PLATO INNER JOIN DETALLE_PEDIDO ON COD_PLATO = PLATO ORDER BY NUM_PEDIDO, 2;