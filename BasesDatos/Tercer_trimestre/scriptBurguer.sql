...
DBMS_OUTPUT.PUT_LINE(RPAD('NUMERO',8)||RPAD('FECHA',30)||'IMPORTE');
CANT := 0;
TOT := 0;
WHILE CEJ5%FOUND LOOP
    DBMS_OUTPUT.PUT_LINE(RPAD(REGEJ5.NUMERO,8)||RPAD(REGEJ5.HORA_TM,30)||REGEJ5.IMPORTE);
    CANT := CANT + 1;
...
SELECT SUM(CANTIDAD) INTO CANT FROM CONSTA WHERE CODIGO_PR = COD;
RETURN CANT;
...
COD VARCHAR(2);
BEGIN
SELECT PRECIO, CODIGO INTO V_PRECIO, COD FROM PRODUCTO WHERE NOMBRE = NOM;
CANT := EJ6(COD);
IF CANT < 5 THEN
...
CREATE OR REPLACE TRIGGER EJ8 after INSERT ON CONSTA 
FOR EACH ROW
DECLARE
MAYOR EXCEPTION;
CANT INT;
BEGIN
SELECT STOCK INTO CANT FROM PRODUCTO WHERE CODIGO = :NEW.CODIGO_PR;
IF :NEW.CANTIDAD > CANT THEN RAISE_APPLICATION_ERROR(-20001, 'no hay stock suficiente');
END IF;
UPDATE PRODUCTO SET STOCK = STOCK - :NEW.CANTIDAD WHERE CODIGO = :NEW.CODIGO_PR;

EXCEPTION 
    WHEN MAYOR THEN
        DBMS_OUTPUT.PUT_LINE('No hay stock suficiente para realizar la operacion');
...
BEGIN 
INSERT INTO CONSTA VALUES('02','0011', 99);
END;

SELECT * FROM CONSTA ORDER BY 1;
